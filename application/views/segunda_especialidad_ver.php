...
$consulta = "SELECT Seg_Esp_Cod,Seg_Esp_Nom,Seg_Est,Esc_Nom FROM segunda_especialidad,escuela WHERE segunda_especialidad.Escuela_Esc_Cod = escuela.Esc_Cod ORDER BY Seg_Esp_Cod ASC";
$resultado = mysqli_query( $conexion, $consulta ) or die ( "Algo ha ido mal en la consulta a la base de datos");
//$mostrar=mysqli_fetch_array($resultado);
while($mostrar=mysqli_fetch_array($resultado)){
	if($mostrar['Seg_Est']==0)
		continue;
?>
		<tr>
			<td><?php echo $mostrar['Seg_Esp_Cod']?></td>
			<td><?php echo $mostrar['Seg_Esp_Nom']?></td>
      <?php
        $var = "Habilitado";
        if($mostrar['Seg_Est']==2)
            $var = "Desistido";
      ?>
