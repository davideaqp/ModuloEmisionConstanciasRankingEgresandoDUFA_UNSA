...
$numero = $_GET['num'];
$sentencia = "SELECT * FROM `grado` WHERE `Gra_Cod`='".$numero."'";
$resultado = mysqli_query($conexion, $sentencia) or die (mysql_error());
$filas=mysqli_fetch_assoc($resultado);

$contenido = [$filas['Gra_Cod'],$filas['Gra_Nom'],$filas['Gra_Est'],$filas['Escuela_Esc_Cod']];
...
$resultado = mysqli_query($conexion, $sentencia) or die (mysql_error());
$filas=mysqli_fetch_assoc($resultado);
$pSegundaEscuela = mysqli_fetch_assoc($resultadoNombreEscuela);
$escuelaM = [$pSegundaEscuela['Esc_Nom']];
?>
<!DOCTYPE html>
<html lang="en">
  <head>
        ...
        <div class="card text-center">
          <div class="card-header">
            <ul class="nav nav-pills card-header-pills">
              <li class="nav-item">
                <a class="btn btn-danger" href="../views/segunda_especialidad_ver.php">Ver Grados Pregrado</a>
              </li>
            </ul>
            </div>
              <div class="card-body">
                <h5 class="card-title">Modificar</h5>
                <h6 class="card-subtitle mb-2 text-muted">Llene el formulario Modificar Grado de Pregrado</h6>
								<form name="actualizar" action="Actualizar_Grado.php" method="post">

                  <div class="form-group row">
                  <label for="gradoCodigo" class="col-sm-2 col-form-label">Codigo:</label>
                  <div class="col-sm-10">
                  <input type="text" name="codigo" class="form-control" id="grado" placeholder="codigo" value="<?php echo $contenido[0] ?>" readonly>
                  </div>
                </div>

                <div class="form-group row">
                  <label for="gradoNombre" class="col-sm-2 col-form-label">Nombre:</label>
                  <div class="col-sm-10">
									<input type="text" name="denominacion" class="form-control" id="graNombre" placeholder="Nombre del Grado" value="<?php echo $contenido[1] ?>">
                  </div>
                </div>

             <div class="form-group row">
                <label for="gradoCodigo" class="col-sm-2 col-form-label">Programa/Escuela:</label>
                <div class="col-sm-8">

                <select name="escuela"  class="form-control">

                  <option selected value="<?php echo $contenido[3] ?>"><?php echo $escuelaM[0]; ?></option>
                  <?php
                    while($mostrar = mysqli_fetch_array($resultadoEscuelas)){ ?>
                    <option value="<?php echo $mostrar["Esc_Cod"]; ?>"><?php echo $mostrar["Esc_Nom"]; ?></option>
                    <?php }?>
                    </select>
